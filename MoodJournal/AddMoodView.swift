...
struct AddMoodView: View {
    ...
    var body: some View {
        VStack(spacing: 30) {
            Text("Add a Journal Entry").font(.largeTitle)
            HStack {
                Button(action: {
            ...
            Button(action: {
                ...
                Text("Add Journal Entry").bold().frame(width: UIScreen.main.bounds.width - 30, height: 50).background(Color.purple).foregroundColor(Color.black).cornerRadius(10)
            }
            Spacer()
        }.padding()
    }
